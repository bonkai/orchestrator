...
| **C6** | **$0 executor** *(Branch A only; build last)* | `spawn.spawn_codex_dispatch` + a write-capable `codex_dispatch_run.sh` (`-s workspace-write`, FIFO so codex's REAL pid is the kill target, PID at the CLAUDE path) + the §5 in-band poller (`app._codex_dispatch_poller`) + the extracted completion core (`app._finalize_dispatch`) + codex cap hard-kill + the summarizer codex branch | ✅ **built + tested (2026-06-23)** |

Build strictly in order; the seat (C2) is the first shippable thing and is hook-gap-free.
**C0–C6 are ALL BUILT + TESTED (2026-06-23); the codex integration is complete.**
...
- [x] **C6.1** ✅ `spawn.spawn_codex_dispatch` (mirrors `spawn_iterm2`) + `spawn.CODEX_DISPATCH_RUN_SH_CONTENT`, a NEW write-capable runner **interpolated from `CODEX_ENGINE_SEED`** (finished the C4-deferred seed→bash interp; pinned by `tests/test_codex_config.py::TestSpawnCodexDispatchRunShPinnedToSeed`). Runs `-s workspace-write` ALONE — **NOT** the `auto_bypass_flag` (C6.0: bypass OVERRIDES `-s` to full-access; workspace-write alone is non-hanging — note 1 was wrong; `executor_sandbox` added to the seed, operator-confirmed confined default, overridable via `fusion.codex.executor_sandbox`). Uses a **FIFO + backgrounded codex** so the recorded PID is codex's REAL pid (a pipeline `$$` would orphan codex on SIGTERM) — written to `PIDS_DIR/<dispatch_id>.pid` (note 2), tab tagged `user.orch_id` + titled `orch #<id>`, `ORCHESTRATOR_CODEX_RUN_ID` (never `ORCHESTRATOR_RUN_ID`), model passed explicitly, `OPENAI_API_KEY` scrubbed, `< /dev/null`. The `_run_dispatch` C5 seam is REPLACED with this spawn + `watchdog.schedule(engine="codex")` + `watchdog.schedule_codex_poller` — never a claude fallback, on success OR spawn failure. · *verified offline (`tests/test_fusion_send.py::TestRunDispatchCodexSeam`, `test_codex_executor.py`); run.sh validated live end-to-end (real-pid capture + clean sidecar + .done + FIFO cleanup, on both success-mechanics and a real codex error).*
- [x] **C6.2** ✅ The §5 fix (iii): `app._codex_dispatch_poller` (a lifetime async task, modeled on the watchdog, factory-injected so `resume_watchers_on_boot` re-attaches it `tail_only=True`) tails the sidecar JSONL → `tool_use`/`tool_result` timeline + `idle_notifier.reset_idle` + `loop_watchdog.record` from the codex fingerprint (`claude_runner._codex_tool_event`, C6.0 schema: `command_execution`/`file_change`, dedup by `item.id`), and on `.done` calls the EXTRACTED completion core `app._finalize_dispatch` IN-PROCESS (no self-POST). The core (shared with `/api/complete`, claude path byte-for-byte) = `watchdog.cancel` → atomic `db.complete_dispatch` (the 4-way race winner) → transcript copy/artifact → `loop_watchdog`/`idle` clear → summarizer. **Summarizer fork (note 4): an additive codex branch in `summarizer.distill_transcript`** (most reversible; real non-empty summary; claude path unchanged). · *verified (`tests/test_codex_executor.py`): tool-event mapper, distillation, timeline+loop-trip-→-kill, poller finalize on .done exit0 / nonzero / closed-tab, and tail_only boot-reattach idempotency.*
- [x] **C6.3** ✅ Every kill/timeout writes an `outcomes` row with reason: manual-kill / kill-all / loop-kill are FREE via note 2 (PID at the claude path → `watchdog.manual_kill` → `db.kill_dispatch_record`), and the **cap hit HARD-KILLS** with a DISTINCT reason — `_run(engine="codex")` skips the claude pause-and-resume (codex has no session_id / no v1 resume, note 5) and records `"timeout (codex cap — hard-kill, not resumable)"`, also cancelling the poller. claude `effort` is ignored for codex (model default). · *verified (`tests/test_codex_executor.py::TestCodexCapWatchdog`): the codex cap hard-kills with the distinct reason + cancels the poller. **Operator-run live checks (server needs a manual restart — reload=False):** watchable tab + manual-kill / global kill-all / cap termination.*
...
**STATUS — C0–C6 are ALL BUILT + TESTED (2026-06-23): the codex integration is complete.
C6 shipped the $0 codex EXECUTOR — `spawn.spawn_codex_dispatch` + a seed-interpolated
write-capable `codex_dispatch_run.sh` (`-s workspace-write`, FIFO→codex's real pid at the
CLAUDE pid path) + the §5 in-band poller (`app._codex_dispatch_poller`) feeding the
timeline + loop watchdog + the extracted completion core (`app._finalize_dispatch`, shared
