...
- [ ] **C5.1** The picker is **engine + per-engine model id**, not a flat toggle. `/send` gains an executor `engine` (+ its model) and accepts `codex` seats in the seat JSON (extend the F9 `fusion_seats` shape with `{type:"codex",model}`); validate against a codex model whitelist. · *verify:* selecting codex+model dispatches a codex executor (C6) / adds a codex seat; **a default that omits the model is rejected** (no silent downgrade).
- [ ] **C5.2** `_view_ctx` surfaces codex availability (`codex_cli_available()`) so the engine option is **disabled** when codex is absent/logged-out, mirroring the `is_fusion_available()` gating. · *verify:* codex absent → option greyed with a reason; present+authed → selectable.
...
- [ ] **C6.1** `spawn.spawn_codex_dispatch` (mirror `spawn_iterm2`): a codex branch of `run.sh` that writes a **PID file** (so `kill_pid` + the wall-clock cap reach it), tags the tab `user.orch_id`, and runs codex with the **auto-bypass flag** from C0 (else an unwatched dispatch **hangs** on an approval prompt) and the model **explicitly**. · *verify:* a dispatch opens a watchable tab; manual kill + global kill-all + the cap all terminate it.
- [ ] **C6.2** The §5 fix (iii): the dispatch runs through the sidecar/PID-poll path; on `.done`, the orchestrator runs the same completion logic `/api/complete` drives (outcome row + summarizer), feeds `loop_watchdog.record` from the streamed JSONL fingerprint, and records timeline events. · *verify:* a codex dispatch produces an outcome row + summary; a looping codex run is auto-killed; the timeline shows tool activity — **all without `~/.claude/settings.json`**.
- [ ] **C6.3** Every kill/timeout writes an `outcomes` row with reason (safety parity with the claude executor). · *verify:* a killed codex dispatch leaves an outcome row the learning loop can see.
