#!/usr/bin/env python3
"""Fusion provider script — xAI Grok (OpenAI-compatible endpoint).
...
later). Stdlib only. Speaks xAI's OpenAI-COMPATIBLE API (`/v1/chat/completions`),
resolves its own key (env → config.json), streams progress + the answer to
...
KEY_ENV, NAME = "XAI_API_KEY", "xai"


def _key():
    """Precedence: env XAI_API_KEY → config.json api_key → '' (mirrors
