#!/usr/bin/env python3
"""Fusion provider script — DeepSeek (OpenAI-compatible endpoint).
...
later). Stdlib only. Speaks DeepSeek's OpenAI-COMPATIBLE API
(`/chat/completions`), resolves its own key (env → config.json), streams
progress + the answer to STDERR (watchable), and prints exactly one line of
normalized JSON to STDOUT. NEVER raises.

Normalized stdout contract (identical for every provider script):
    {"ok": true, "text": "...", "model": "deepseek-chat",
     "prompt_tokens": 0, "completion_tokens": 0, "error": ""}

Request — argv[1] is a path to a JSON file, or "-"/absent reads STDIN:
    {"prompt": "...", "model": "deepseek-chat", "timeout_s": 300}
"""
import json
import os
import sys
...
KEY_ENV, NAME = "DEEPSEEK_API_KEY", "deepseek"


def _key():
    """Precedence: env DEEPSEEK_API_KEY → config.json api_key → '' (mirrors
